...
export function useRoadmap() {
  ...
  const toggleTask = async (phaseId: string, taskId: string, done: boolean, userId: string) => {
    ...
    // Fetch the task info to write evidence record
    ...
    await supabase
      .from("roadmap_tasks")
      .update({ done, status, completed_at: completedAt })
      .eq("id", taskId);
    ...
    if (done) {
      await supabase.from("activity_log").insert({
        user_id: userId,
        type: "task_completed",
        description: `Completed roadmap task: ${taskData?.name || "Task"}`
      });

      if (taskData?.skill_name) {
        await supabase.from("evidence_records").insert({
          user_id: userId,
          skill_name: taskData.skill_name,
          source_type: "manual",
          excerpt: `Self-reported completion of roadmap task: ${taskData.name}`,
          confidence: 40 // modest confidence boost
        });

        // Trigger recalculation of skill confidence
        await supabase.rpc("calculate_skill_confidence", {
          p_skill_name: taskData.skill_name,
          p_user_id: userId
        });
      }
    }
    ...
    setPhases((prev) => prev.map((p) => {
      ...
      const phaseStatus = pct === 100 ? "completed" : pct > 0 ? "active" : "pending";
      supabase.from("roadmap_phases").update({ progress: pct, status: phaseStatus }).eq("id", phaseId);
      
      return { ...p, tasks: newTasks, progress: pct, status: phaseStatus };
    }));
  };
